...
// eraseCells erases the cells of the area of cols x rows cells starting at
// screen row top with the default background, except the cells covered by
// the keep rectangles (screen cell coordinates). Used to remove a sixel
// image the cell renderer doesn't know about.
func eraseCells(top, cols, rows int, keep []image.Rectangle) string {
	if cols <= 0 || rows <= 0 {
		return ""
	}
	var sb strings.Builder
	...
	for y := top; y < top+rows; y++ {
		...
		for x < cols {
			// skip kept cells
			if r, ok := covering(keep, x, y); ok {
				x = r.Max.X
				continue
			}
			end := cols
			for _, r := range keep {
				if y >= r.Min.Y && y < r.Max.Y && r.Min.X > x && r.Min.X < end {
					end = r.Min.X
				}
			}
			fmt.Fprintf(&sb, "\x1b[%d;%dH\x1b[%dX", y+1, x+1, end-x)
			x = end
		}
	}
	sb.WriteString("\x1b8")
...
func covering(rs []image.Rectangle, x, y int) (image.Rectangle, bool) {
	for _, r := range rs {
